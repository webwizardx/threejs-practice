...
class PCDLoader extends Loader {
	...
	load(
		url: string,
		onLoad: (geometry: Points<BufferGeometry, PointsMaterial>) => void,
	...
	parse(data: any) {
		...
		function parseHeader(data: string) {
			...
			if (PCDheader.size !== null) {
				PCDheader.size = PCDheader.size[1]
					.split(' ')
					.map(function (x: any) {
		...
		if (PCDheader.data === 'binary_compressed') {
			...
			for (let i = 0; i < PCDheader.points; i++) {
				if (offset.x !== undefined) {
					...
					position.push(
						dataview.getFloat32(
							PCDheader.points * offset.x +
								PCDheader.size[xIndex] * i,
							this.littleEndian
						)
					);
					position.push(
						dataview.getFloat32(
							PCDheader.points * offset.y +
								PCDheader.size[yIndex] * i,
							this.littleEndian
						)
					);
					position.push(
						dataview.getFloat32(
							PCDheader.points * offset.z +
								PCDheader.size[zIndex] * i,
							this.littleEndian
						)
					);
				}
